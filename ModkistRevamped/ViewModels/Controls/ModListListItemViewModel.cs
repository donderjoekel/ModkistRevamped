...
public partial class ModListListItemViewModel : ObservableObject
{
    ...
    public ModListListItemViewModel(
    ...
    )
    {
        ...
        Title = Mod.Name!;
        Information =
            $"{Mod.SubmittedBy?.Username ?? "Unknown author"} | {Mod.Modfile?.Version ?? "Unknown version"} | {DateTimeOffset.FromUnixTimeSeconds(Mod.DateUpdated).Humanize()}";
        UpdateView();
        LoadImage();
    }
